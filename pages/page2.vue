<template>
  <div class="relative flex items-top justify-center min-h-screen bg-gray-100 sm:items-center sm:pt-0">
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 min-w-full">
      <a class="flex justify-center pt-0" href="https://nuxtjs.org" target="_blank">
      ...
      <h2 class="mt-2 text-xl text-center font-semibold">
        Page 2
      </h2>
      <div>
  ...
  methods: {
    sendWebSocketMessage() {
      ...
      this.$qewd.reply({
        type: 'test',
        params: {
          text: 'a NuxtJS WebSockets test message from page 2'
        }
      }).then(response => {
    ...
    async sendRESTMessage() {
      try {
        const response = await this.$http.$get(`${this.$config.qewdUrl}/api/test?text=a+NuxtJS+REST+test+message+from+page+2`)
